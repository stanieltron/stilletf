...
export const authOptions = {
  ...
  // Ensure cookie prefixes/options stay consistent behind reverse proxies (Railway, etc.)
  ...
  callbacks: {
    async redirect({ url, baseUrl }) {
      const canonicalBaseUrl = process.env.NEXTAUTH_URL || baseUrl;

      if (url.startsWith("/")) return `${canonicalBaseUrl}${url}`;

      try {
        const parsed = new URL(url);
        if (parsed.origin === baseUrl || parsed.origin === canonicalBaseUrl) {
          return `${canonicalBaseUrl}${parsed.pathname}${parsed.search}${parsed.hash}`;
        }
      } catch {
        // Ignore malformed URLs and fall back to canonical base URL.
      }
