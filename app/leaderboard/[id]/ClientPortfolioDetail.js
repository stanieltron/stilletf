...
import ShareModalSignedIn from "../../components/ShareModalSignedIn";
...
export default function ClientPortfolioDetail({ id }) {
  ...
  }, [p?.isOwner, p?.user?.id, p?.userId, userId]);
  const userDisplay = useMemo(() => {
    ...
    return p?.nickname || "";
  }, [p?.nickname, session?.user?.email, session?.user?.name, session?.user?.nickname]);
  const comp = useMemo(
            ...
            <div className="flex flex-wrap items-center gap-3 sm:gap-4">
              ...
              {isOwner && (
                <button
                  ...
                  className="px-4 sm:px-5 py-2 rounded-[10px] border border-[#201909] bg-[#201909] text-white text-sm sm:text-base font-semibold tracking-tight shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md"
                  title="Share portfolio to gain future rewards"
                >
                  Share portfolio to gain future rewards
                </button>
              )}
