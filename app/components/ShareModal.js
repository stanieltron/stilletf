    ...
    <section className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
      ...
      >
        ...
        <div className="px-6 pb-6 pt-4 flex flex-col items-center gap-4">
          ...
          {isMobile ? (
            ...
            <div className="w-full flex gap-3">
              ...
              <button
                type="button"
                onClick={openSignIn}
                className="h-11 flex-1 rounded-xl border border-[rgba(255,255,255,0.2)] bg-transparent text-white/80 text-sm font-semibold hover:text-white hover:border-[rgba(255,255,255,0.35)] transition-colors active:scale-[0.98]"
          ...
          {!!error && (
            <div className="text-sm text-rose-300 w-full">{error}</div>
          )}
          {!imgDataUrl && !error && (
